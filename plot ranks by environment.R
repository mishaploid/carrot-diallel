...
lapply(ranks_wi15, function(x) write.table( data.frame(x), 
                                            "~/Documents/carrot-diallel-all-files/BayesDiallel/ranks_wi15.csv", 
                                            append= T, sep="," ))

for (i in varNames) {
  filename = paste("~/Documents/carrot-diallel-all-files/Bayesdiallel/wi2015/", i, "_ranks_wi2015.csv")
  write.csv(ranks_wi15[[i]], filename)
}

for (i in varNames) {
  filename = paste("~/Documents/carrot-diallel-all-files/Bayesdiallel/ca2015/", i, "_ranks_ca2015.csv")
  write.csv(ranks_ca15[[i]], filename)
}

for (i in varNames) {
  filename = paste("~/Documents/carrot-diallel-all-files/Bayesdiallel/ca2016/", i, "_ranks_ca2016.csv")
