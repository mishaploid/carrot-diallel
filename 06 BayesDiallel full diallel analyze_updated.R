...
Diallel.tau.Prior.Info <- read.csv("Diallel tau prior.csv", header = TRUE, 
                                   row.names = 1)
...
# 'fullu' model w/ year and stand as random effects
# ------------------------------------------------------------------------------
results <- list()

for (i in varNames) {
  dir.create(paste0("~/Documents/carrot-diallel/BayesDiallel/", i, sep = ""))
